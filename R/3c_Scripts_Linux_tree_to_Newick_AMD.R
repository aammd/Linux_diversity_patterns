...
# ---------------------------------------------------------------
# creating some auxiliary files useful for trait data acquisition
# ---------------------------------------------------------------

# extract tip labels from the Debian tree (so that they can be
# mapped to the package list files from Distrowatch)
Debian_tree <- read.tree(text=paste0(all_trees[[1]], sep=";"))
write.csv(data.frame(gldt_name=sort(Debian_tree$tip.label)), 
          file="data/Debian_phylo_labels.csv",
          row.names=FALSE)

# create empty file for each distro
for(distro in Debian_tree$tip.label)
{
  file.create(file=paste("data/Debian_traits/", distro, sep=""))
}
...
pdf("figures/Figure_3A.pdf", width=10, height=5)
  par(mfrow=c(1,3), mai=c(0.1,0.3,0.3,0.1))
  titles <- c("Debian", "SLS", "Red Hat")
  for(i in c(1,3,2))
  {
    
    plot_tree(all_trees[[i]], 
              show.tip.label = FALSE, 
              main=titles[i],
              font.main=1)  
    if(i==1){mtext("A", adj=-0.05,  font=2)}
  }
dev.off()
